...
- **Player Elo leaderboard** — an opponent-adjusted rating built purely from
  ...
  **role-weighted** blend of attack (xG + 0.7 × xA), defence (tackles +
  ...
  on a forward's xG bar (see `docs/models.md`). International appearances
  move the rating too (opponent = the national team's Elo, friendlies at half
  weight), but a player is always listed at his club. Ratings carry across
  seasons with no reset; a player who stops featuring bleeds toward 1500 for every match his
